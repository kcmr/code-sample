...
export function render(result, container, templateFactory = defaultTemplateFactory) {
  ...
  let instance = container.__templateInstance; // Repeat render, just call update()
...
export class Template {
  constructor(result, element) {
    ...
    const content = this.element.content; // Edge needs all 4 parameters present; IE11 needs 3rd parameter to be null
    ...
    let index = -1;
    let partIndex = 0;
    const nodesToRemove = []; // The actual previous node, accounting for removals: if a node is removed
    ...
    while (walker.nextNode()) {
      ...
      /* Node.ELEMENT_NODE */
          ...
          const attributes = node.attributes; // Per https://developer.mozilla.org/en-US/docs/Web/API/NamedNodeMap,
          ...
          while (count-- > 0) {
            ...
            const attributeNameInPart = lastAttributeNameRegex.exec(stringForPart)[1]; // Find the corresponding attribute
      ...
      && node.nodeValue === marker) {
        const parent = node.parentNode; // Add a new marker node to be the startNode of the Part if any of the
        // following are true:
...
export class NodePart {
  ...
  _setIterable(value) {
    ...
    for (const item of value) {
      ...
      let itemPart = itemParts[partIndex]; // If no existing part, create a new one

      if (itemPart === undefined) {
        // If we're creating the first item part, it's startNode should be the
        // container's startNode
        let itemStart = this.startNode; // If we're not creating the first part, create a new separator marker
    ...
    if (partIndex === 0) {
      ...
      const lastPart = itemParts[partIndex - 1]; // Truncate the parts array so _previousValue reflects the current state
